...
async fn scan_service() -> HttpResponse {
	...
	let scan_stream = scan_stream_bmp().unwrap();
...
fn scan_stream_bmp(
) -> anyhow::Result<Box<dyn Stream<Item = Result<Bytes, anyhow::Error>> + Unpin + 'static>> {
	let (sender, receiver) = tokio::sync::mpsc::unbounded_channel();

	tokio::task::spawn_blocking(move || {
		let libsane = match unsafe { sane::libsane::new("libsane.so.1") } {
			Ok(v) => v,
		...
		let status: sane::SANE_Status =
			unsafe { libsane.sane_init(&mut version_code as *mut sane::SANE_Int, None) };
		if status != sane::SANE_Status_SANE_STATUS_GOOD {
			return sender.send(Err(anyhow!("Sane status is not good: {}", status)));
		}
		...
		if status != sane::SANE_Status_SANE_STATUS_GOOD {
			return sender.send(Err(anyhow!("Failed to get devices {}", status)));
		}
		let device_count = unsafe {
			let mut device_count = 0_usize;
			while !(*device_list.add(device_count)).is_null() {
		...
		if device_list.is_empty() {
			return sender.send(Err(anyhow!("No scanners found.")));
		}
		...
		if status != sane::SANE_Status_SANE_STATUS_GOOD {
			return sender.send(Err(anyhow!("Failed to open device {}.", status)));
		}
		let device_handle = unsafe { Device::from_raw_handle(device_handle, &libsane) };
		...
		if status != sane::SANE_Status_SANE_STATUS_GOOD {
			return sender.send(Err(anyhow!(
				"Failed to get control option (option count) {}",
				status
			)));
		}
		debug!("Number of available options: {}.", option_count_value);
		...
		for option_num in 1..option_count_value {
			let option_descriptor =
				unsafe { libsane.sane_get_option_descriptor(device_handle.0, option_num) };
			if option_descriptor.is_null() {
				return sender.send(Err(anyhow!("Failed to get option count")));
			}
			...
			unsafe {
				...
				match (*option_descriptor).constraint_type {
					...
					_ => return sender.send(Err(anyhow!("Unknown constraint type"))),
				}
			}
			...
			unsafe {
				...
				if name == CStr::from_bytes_with_nul_unchecked(b"resolution\0") {
					...
					let mut value = match (*option_descriptor).type_ {
						sane::SANE_Value_Type_SANE_TYPE_INT
						| sane::SANE_Value_Type_SANE_TYPE_FIXED => {
							// 300 DPI
							300_i32
						}
						type_ => {
					...
					if status != sane::SANE_Status_SANE_STATUS_GOOD {
						return sender.send(Err(anyhow!(
							"Failed to fetch back set dpi setting {}.",
							status
						)));
					}
					info!("DPI set to {}.", dpi);
		...
		if status != sane::SANE_Status_SANE_STATUS_GOOD {
			return sender.send(Err(anyhow!(
				"Failed to initiate image acquisition {}.",
				status
			)));
		}
		...
		if status != sane::SANE_Status_SANE_STATUS_GOOD {
			return sender.send(Err(anyhow!("Failed to get scan parameters {}.", status)));
		}
		...
		if let Err(e) = encode_as_bmp(width * height * 3, (width, height), &mut bmp_header) {
			return sender.send(Err(e.into()));
		}
		sender.send(Ok(Bytes::from(bmp_header)))?;
		...
		loop {
			...
			match status {
				...
				status => {
					return sender.send(Err(anyhow!("Failed to read data {}.", status)));
				}
			}
			...
			sender.send(Ok(Bytes::from(cloned_buf)))?;
		}

		unsafe {
			libsane.sane_cancel(device_handle.0);
		}
		Result::<(), SendError<_>>::Ok(())
	});
	Ok(Box::new(StreamingReceiver(receiver)))
}
